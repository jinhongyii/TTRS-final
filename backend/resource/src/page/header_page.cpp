...
bool HeaderPage::InsertRecord(const std::string &name, const page_id_t root_id,
                              Transaction *txn, LogManager *logManager) {
...
//  assert(root_id > INVALID_PAGE_ID);
  ...
  if (ENABLE_LOGGING) {
      sjtu::string<32> str(name);
      LogRecord logRecord(txn->GetTransactionId(),txn->GetPrevLSN(),LogRecordType::INSERTROOT,str,root_id);
...
bool HeaderPage::GetRootId(int id,page_id_t& root_id) {
...
bool HeaderPage::UpdateRecord(int RecordId,page_id_t rootid) {
    int index=RecordId;
  if (index == -1)
    return false;
  int offset = index * 36 + 4;
  // update record content, only root_id
  memcpy((GetData() + offset + 32), &rootid, 4);
}
} // namespace sjtu
